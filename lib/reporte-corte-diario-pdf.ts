...
type CorteDiario = {
  ...
  diferencia_cobrado_recibido?: number | string;
  total_pendiente_cobro?: number | string;
  por_asesor?: any[];
  pagos?: any[];
};
...
export function exportarCorteDiarioPdf(data: CorteDiario): void {
  ...
  const asesoresHtml = asesores.map((asesor, index) => {
    ...
    const abonosAtrasados = pagosAsesor.filter((pago) => montoAtrasado(pago) > 0.009);
    const abonosMora = pagosAsesor.filter((pago) => foliosMora.has(String(pago.credito?.num_prog ?? pago.num_prog)));
    const total = (items: any[], monto = (item: any) => item.monto) => items.reduce((sum, item) => sum + Number(monto(item) ?? 0), 0);
    const totalPendiente = Number(asesor.monto_pendiente_cobro ?? rutaPendiente.reduce((sum: number, item: any) => sum + Number(item.monto_a_cobrar ?? 0), 0));
  ...
  <title>Corte diario ${escapeHtml(fecha)}</title><style>
    ...
    * { box-sizing: border-box; } body { font-family: Arial, Helvetica, sans-serif; color: #172033; font-size: 8pt; margin: 0; background: transparent; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    ...
    .section { margin-top:8px; break-inside:avoid; } .section-title { display:flex; justify-content:space-between; padding:4px 6px; color:white; font-size:8pt; font-weight:700; } .green { background:rgba(21, 128, 61, .9); } .blue { background:rgba(37, 99, 235, .9); } .purple { background:rgba(109, 40, 217, .9); } .amber { background:rgba(180, 83, 9, .9); } .red { background:rgba(185, 28, 28, .9); }
    ...
    <section class="summary"><div><span>Ruta programada</span><strong>${money(data.total_programado_dia)}</strong></div><div><span>Cobrado App</span><strong>${money(data.total_abonos)}</strong></div><div><span>Diferencia por entregar</span><strong>${money(data.diferencia_cobrado_recibido)}</strong></div><div><span>Monto pendiente</span><strong>${money(data.total_pendiente_cobro)}</strong></div></section>
