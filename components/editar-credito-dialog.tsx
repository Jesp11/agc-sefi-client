...
type CreditoEditable = Record<string, unknown> & {
  ...
  dia_quincena_1?: number | null;
  dia_quincena_2?: number | null;
  movimiento_en_proceso?: { estado: string; categoria?: string | null; descripcion_estado?: string } | null;
};
...
export function EditarCreditoDialog({ credito, onSuccess }: EditarCreditoDialogProps) {
  ...
  const esQuincenal = credito.frecuencia_pago === "Quincenal";
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        // En quincenales el día de pago ("15 Y 30") lo deriva la API de los días del mes.
        ...(esQuincenal ? {} : { dias_pago: form.dias_pago }),
  ...
  // Editar mientras el desembolso sigue en Movimientos desincroniza caja y cartera (la API también lo rechaza).
  const movimientoEnProceso = credito.movimiento_en_proceso;
  if (movimientoEnProceso) {
    return (
      <span
        className="cursor-not-allowed"
        title={`Movimiento de ${String(movimientoEnProceso.categoria ?? "desembolso").toLowerCase()} ${movimientoEnProceso.descripcion_estado ?? movimientoEnProceso.estado}. Confírmalo o cancélalo en Movimientos para editar.`}
      >
        <Button variant="outline" size="sm" className="h-9 gap-1.5 font-semibold text-xs" disabled>
          <Pencil className="h-3.5 w-3.5" />Editar crédito
        </Button>
      </span>
    );
  }
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              ...
              {esQuincenal ? (
                <div className="grid gap-1.5">
                  ...
                  <Input value={`Quincenal: ${credito.dia_quincena_1 ?? 15} y ${credito.dia_quincena_2 ?? 30} de cada mes`} disabled />
                </div>
              ) : (
                <div className="grid gap-1.5">
                  <Label>Día de pago</Label>
